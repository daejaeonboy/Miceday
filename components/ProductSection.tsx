  ...
  return (
    <div className={`py-12 md:py-20 ${variant === 'gray' ? 'bg-slate-50' : 'bg-white'}`}>
      <Container>
        ...
        {/* Filter Pills */}
        <div className="flex overflow-x-auto no-scrollbar gap-4 mb-6 whitespace-nowrap px-1">
          {categories.map((cat, idx) => (
            <button
              key={`${cat}-${idx}`}
              onClick={() => setActiveCategory(cat)}
              className={`h-[40px] min-w-[100px] px-4 rounded-lg text-[14px] md:text-[15px] font-bold transition-all border flex-shrink-0 text-center shadow-sm hover:shadow
                ${activeCategory === cat
        ...
        <div className="relative group">
          ...
          <div
            ...
            className="flex gap-8 overflow-x-auto no-scrollbar scrollbar-hide [&::-webkit-scrollbar]:hidden pb-4"
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    {product.discountRate && (
                      <span className="text-rose-500 font-bold text-sm md:text-base">{product.discountRate}%</span>
                    )}
